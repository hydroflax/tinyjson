...
public:
	...
	unsigned int location; // The current location of the user. (Used by parse). (Enables user to insert K-V pairs into the current JSON string).
	char* json; // The JSON string.

	char* key; // A pointer to the start of the key (ignoring quotation marks). This is used know where the user has read up to in the JSON string stored in memory. (next() uses this to the next key-value pair).
	 ...
	 * @param unsigned int maxSize	- Specify a maximum size that will be used by the JSON string (if known). If not chosen, the string will be of variable size (need to check to ensure string will be stored in a row).
	 */
	 ...
	 *
	 * @param bool store			- Store in IRAM or in EEPROM (Depending on purpose). (1 = EEPROM, 0 = IRAM).
	 * @param bool ignoreQuotes		- Ignore quotation marks, saving space in memory. (To Be Implemented at a later date).
	 * @param unsigned int maxSize	- Specify a maximum size that will be used by the JSON string (if known). If not chosen, the string will be of variable size (need to check to ensure string will be stored in a row).
	 */
	 ...
	 * During the read, remove any whitespace, /r, /n characters to ensure the smallest possible stored JSON. (Potential to remove the quotation marks as well).
	...
	// This allows for users to provide an already formatted JSON key-value pair or just the key, value. (Unsure at this point as to how to handle array values/embedded key-value pairs).
	...
	// user can get current Key-Value Pair (depending on where next is) by looking at the 'tinyjsonpp.key' and 'tinyjsonpp.value' properties. They will also be able to look at the 'tinyjsonpp.valueType', which will
	...
	// Function to empty the current JSON string.
};
